<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<div id="header" class="header header-note">
	<div class = "header-wrapper ht">
		<div class="logo">
			<a href="http://10.186.106.131:8081/mo" ><img src="${ctxStatic }/image/logo.png" alt="LOGO 默" title="LOGO MO"/></a>
		</div>
		<div class="nav">
			<ul class="nav-ul">
				<li class="nav-ul-item"><a href="http://10.186.106.131:8081/mo">随笔记事</a>
					<!-- <div class="subnav subnav-notepad"> just do it </div> -->
				</li>
				<li class="nav-ul-item"><a href="http://10.186.106.131:8081/mo/leisure">闲暇小憩</a></li>
			</ul>
		</div>
		<div class="user">
			<a href="http://10.186.106.131:8081/mo/login?type=login">登录</a>&nbsp;|&nbsp;<a href="http://10.186.106.131:8081/mo/login?type=register">注册</a>
		</div>
	</div>
</div>